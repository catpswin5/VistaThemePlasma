...
function uninstall_plasmoid {
    ...
    if [[ $PLASMOID == 'io.gitgud.catpswin56.systemtray' ]]; then
...
if [ "$?" == 1 ]; then
    ...
    uninstall_component "authuiVista" "Plasma/LookAndFeel"
    uninstall_component "io.gitgud.catpswin56.taskbar" "Plasma/LayoutTemplate"
    uninstall_component "Vista-Black" "Plasma/Theme"
    uninstall_component "io.gitgud.catpswin56.desktop" "Plasma/Shell"
fi
...
uninstall_prompt "Kvantum theme" "$KV_DIR/WindowsVistaKvantum_Aero"
if [ "$?" == 1 ]; then
    echo "Uninstalling Kvantum theme..."
    kvantummanager --set KvAdapta
    rm -r "$KV_DIR/WindowsVistaKvantum_Aero"
    echo "Done."
fi
...
if [ "$?" == 1 ]; then
    ...
    rm -r "$ICONS_DIR/Windows Vista Aero"
    echo "Done."
fi
...
if [ ! -z "$(grep -rni "vistathemeplasma" "$BRANDING_DIR/kcm-about-distrorc")" ]; then
...
uninstall_prompt "Plasma applets (requires admin privileges)" "${APPLET_DIR}/io.gitgud.catpswin56."*
if [ "$?" == 1 ]; then
