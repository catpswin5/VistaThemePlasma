...
PlasmaCore.Dialog {
    ...
    readonly property int slide: Kirigami.Units.mediumSpacing*2
    ...
    onVisibleChanged: {
        if(visible) {
            ...
            xpos = visualParent.x + diff + visualParent.width/2 - Kirigami.Units.smallSpacing;
            xpos -= menuWidth / 2;
    ...
    function show() {
        ...
        visible = true;
        opacity = 1;
        tasksMenu.y -= slide;//Kirigami.Units.smallSpacing*2;
        openTimer.start();
    }
    Task {
        id: taskFunc
    ...
    function delayedMenu(delay, func) {
        Plasmoid.disableBlurBehind(tasksMenu);
        tasksMenu.y += Kirigami.Units.smallSpacing*2;
        opacity = 0;
    ...
    FocusScope {
        ...
        Connections {
            ...
            function onMouseEventDetected(mouse) {
                if(!fscope.contains(Plasmoid.getPosition(tasksMenu))) {
                    tasksMenu.closeMenu();
                }
            }
